/*
 *
 * ©K. D. Hedger. Wed 20 Nov 14:36:23 GMT 2024 keithdhedger@gmail.com

 * This file (LFSTKFindClass.cpp) is part of xdg-desktop-portal-filechooser.
...
/*
									...
									if(this->LFSTK_getFileTypes().empty()==false)
										{
											skip=true;
											std::string ts=this->LFSTK_getFileTypes();
											int f=ts.find_first_of("*");
											ts=this->LFSTK_getFileTypes().substr(f,std::string::npos );
											fprintf(stderr,"%s\n",this->LFSTK_getFileTypes().c_str());
											fprintf(stderr,"%s\n",ts.c_str());
								
											std::vector<std::string> tokenstrings=LFSTK_UtilityClass::LFSTK_strTok(this->LFSTK_getFileTypes(),"*");
											//std::vector<std::string> tokenstrings=LFSTK_UtilityClass::LFSTK_strTok(ts," ");
											for(unsigned j=0;j<tokenstrings.size();j++)
												{
												//fprintf(stderr,"j=%i %s\n",j,tokenstrings.at(j).c_str());
													if(LFSTK_UtilityClass::LFSTK_hasSuffix(entry->d_name,tokenstrings.at(j))==true)
														{
														fprintf(stderr,">>name=%s suffix=>>%s<<\n",entry->d_name,tokenstrings.at(j).c_str());
															skip=false;
									...
									if(this->LFSTK_getFileTypes().empty()==false)
										{
											skip=true;
											std::vector<std::string> tokenstrings=LFSTK_UtilityClass::LFSTK_strTok(this->LFSTK_getFileTypes(),"*");
											for(unsigned j=1;j<tokenstrings.size();j++)
												{
													if(LFSTK_UtilityClass::LFSTK_hasSuffix(entry->d_name,LFSTK_UtilityClass::LFSTK_strStrip(tokenstrings.at(j),"( )"))==true)
														{
															skip=false;
															j=tokenstrings.size()+1;
														}
